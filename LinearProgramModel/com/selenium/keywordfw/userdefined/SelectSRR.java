...
public class SelectSRR {
	...
	public static boolean SelectSRR(String list[]){
	columnName=list[2];
	columnValue=list[3];
	boolean status = false;


	List<WebElement>Columns=driver.findElements(By.xpath("//div[@id='srr_container']/div/div[2]/div/table/thead/tr/th/div"));
	...
	private static boolean checkColumnValue(int colId) {
		...
		do {
			...
			List<WebElement>NoOfRecords=driver.findElements(By.xpath("//*[@id='srr_container_grid']/tbody/tr"));
			for (int i = 2; i <= NoOfRecords.size(); i++) {
				List<WebElement>Columns=driver.findElements(By.xpath("//*[@id='srr_container_grid']/tbody/tr["+i+"]/td"));				
					WebElement iCol = Columns.get(colId);
					if(columnValue.equals(iCol.getText())){
						found = true;
						//driver.findElement(By.xpath("//id('srr_container_grid')/tbody/tr["+i+"]/td")).click();
						driver.findElement(By.xpath("//*[@id='srr_container_grid']/tbody/tr["+i+"]/td[1]/div/input")).click();	    				
						break;
					}		
			}
